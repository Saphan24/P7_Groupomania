            ...
            <b-jumbotron class="col-md-6">
                ...
                <form @submit.prevent="signup">
                    <b-form-group>
                        ...
                        <b-form-input type="email" class="form-control" id="email" name="email" v-model="form.email" placeholder="name@example.com" :class="{ 'is-invalid': submitted && $v.form.email.$error }"></b-form-input>
                        <div v-if="submitted && !$v.form.email.required" class="invalid-feedback">Veuillez saisir un email valide</div>
                    </b-form-group>
                    <b-form-group>
                        <label for="username">Username</label>
                        <input type="text" class="form-control" id="username" name="username" v-model="form.username" placeholder="Username" :class="{ 'is-invalid': submitted && $v.form.username.$error }" />
                        <div v-if="submitted && !$v.form.username.required" class="invalid-feedback">Veuillez saisir un username</div>
                    </b-form-group>
                    <b-form-group>
                        <label for="password">Password</label>
                        <input type="password" class="form-control" id="password" name="password" v-model="form.password" placeholder="••••••••••" :class="{ 'is-invalid': submitted && $v.form.password.$error }" />
                        <div v-if="submitted && $v.form.password.$error" class="invalid-feedback">
                            <span v-if="!$v.form.password.required">Veuillez rentrer un mot de passe valide</span> 
                            <span v-else-if="!$v.form.password.minLength">Le mot de passe doit être au moins de 6 caractères</span>
                            <span v-else-if="!$v.form.password.alphaNumberSpecial">Le mot de passe doit  au moins avoir une minuscule, une majuscule, un chiffre et un caractère spécial</span>
                        </div>
                    </b-form-group>
                    <b-form-group>
                        <label for="confirmPassword">Confirm Password</label>
                        <input type="password" class="form-control" id="confirmPassword" name="confirmPassword" v-model="form.confirmPassword" placeholder="••••••••••" :class="{ 'is-invalid': submitted && $v.form.confirmPassword.$error }" />
                        <div v-if="submitted && $v.form.confirmPassword.$error" class="invalid-feedback">
                            <span v-if="!$v.form.confirmPassword.required">Veuillez confirmer le mot de passe</span>
                            <span v-else-if="!$v.form.confirmPassword.sameAsPassword">le mot de passe doit correspondre</span>
                        </div>
                    </b-form-group>
                    <b-form-group>
                        <label>Description</label>
                        <textarea type="text" class="form-control" id="bio" name="bio" v-model="form.bio" rows="4" placeholder="Description" :class="{ 'is-invalid': submitted && $v.form.username.$error }"></textarea>
                        <div v-if="submitted && !$v.form.bio.required" class="invalid-feedback">Veuillez saisir une description de vous</div>
                    </b-form-group>
                    <b-form-group>
                        <button class="btn btn-dark btn-lg btn-block">S'inscrire</button>
                    </b-form-group>
                </form>
            </b-jumbotron>
        </div>
